...
class STNE(object):
    def __init__(self, hidden_dim, node_num, fea_dim, seq_len, depth=1, node_fea=None, node_fea_trainable=False, is_training=True):
        ...
        self.loss_local=tf.reduce_mean(tf.square(tf.subtract(self.enc[:,1:,:],self.enc[:,:-1,:])))
...
#        self.y_smoothed = label_smoothing(tf.one_hot(self.input_seqs, depth=node_num))
        ...
        self.loss_ceall=self.loss_ce+self.loss_local      
...
#        train_op = tf.train.GradientDescentOptimizer(lr).minimize(model.loss_ce, global_step=model.global_step)
        train_op = tf.train.MomentumOptimizer(lr,momentum=0.9).minimize(model.loss_ceall, global_step=model.global_step)
        ...
        for epoch in range(epc):
            ...
            print('Epoch,\tStep,\tLoss_all,\tLoss_tr,\tLoss_loc,\tacc, #Trained Nodes')
            while end_idx < len(node_seq):
                _, loss_all,loss_tr,loss_loc, step,acc = sess.run([train_op, model.loss_ceall,model.loss_ce,model.loss_local, model.global_step,model.acc], feed_dict={
...
#            sess.run(tf.get_default_graph().get_tensor_by_name('dense/kernel:0'))
                ...
                if step % 10 == 0:
                    print(epoch, '\t', step, '\t', loss_all,'\t', loss_tr,'\t', loss_loc,'\t', acc, '\t', len(trained_node_set))
#                    if all_trained:
#                        f1_mi = []
