...
class STNE(object):
    def __init__(self, hidden_dim, node_num, fea_dim, seq_len, depth=1, node_fea=None, node_fea_trainable=False, is_training=True,normed_adj=None):
        ...
        if node_fea is not None:
            ...
            self.normed_matrix=tf.Variable(initial_value=normed_adj, name='normed_matrix', trainable=node_fea_trainable)
        else:
        ...
        self.ch_num=8
        self.adj_keep_r=0.999
        self.keep_r=0.16
        self.is_train=True
        self.k=8
        self.layer_num=2
#        self.class_num=hp.hidden_units
        self.class_num=256
        ...
        outs = ops.simple_conv(
            self.normed_matrix, self.embedding_W, 4*self.ch_num, self.adj_keep_r,
            self.keep_r, self.is_train, 'conv_s', act_fn=True)
#        for layer_index in range(self.layer_num):
#            cur_outs= ops.graph_conv(
#                self.normed_matrix, outs, self.ch_num, self.adj_keep_r,
#                self.keep_r, self.is_train, 'conv_%s' % (layer_index+1),
#                act_fn=None, k=self.k)
#            outs = tf.concat([outs, cur_outs], axis=1, name='concat_%s' % layer_index)
#        outs = ops.simple_conv(
#            self.normed_matrix, outs, self.class_num, self.adj_keep_r,
#            self.keep_r, self.is_train, 'conv_f', act_fn=None, norm=False)
        ...
        self.input_seq_embed = tf.nn.embedding_lookup(outs, self.input_seqs, name='input_embed_lookup')   ##shape(?,10,1433)
...
#    node_seq = read_node_sequences(folder + 'node_sequences_10_10.txt')
    ...
    X=all_data['X']
    Y=all_data['Y']
    node_fea=all_data['node_fea']
    node_seq=all_data['node_seq']
    adj=all_data['adj']

    
    normed_adj = preprocess_adj(adj)
    normed_adj=np.array(normed_adj,dtype='float32')
    node_fea = preprocess_features(node_fea, False)
...
#    gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.7)
    ...
    with tf.Session() as sess:
        model = STNE(hidden_dim=h_dim, node_fea_trainable=trainable, seq_len=s_len, depth=dpt, node_fea=node_fea,
                     node_num=node_fea.shape[0], fea_dim=node_fea.shape[1],normed_adj=normed_adj)
